...
namespace DataIsolationSample
{
    public class Startup
    {
        ...
        private void SetupDb()
        {
            ...
            ti = new TenantInfo("xit", null, null, "Data Source=Data/xit.db", null);
            using (var db = new ToDoDbContext(ti))
            {
                db.Database.EnsureDeleted();
                db.Database.EnsureCreated();
                db.ToDoItems.Add(new ToDoItem { Title = "nopAccelerate", Completed = false });
                db.ToDoItems.Add(new ToDoItem { Title = "nopAccelerate +", Completed = true });
                db.ToDoItems.Add(new ToDoItem { Title = "nopAccelerate Themes", Completed = false });
                db.SaveChanges();
            }
        }
